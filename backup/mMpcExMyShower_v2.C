...
//use the local property  
  cout<<"starting showers2 "<<endl;
  ...
  std::set<unsigned int> peak_find_list;
  //each hits associate with a peak;
  std::map<unsigned int,unsigned int> hits_peak_map;
  for(unsigned int arm = 0;arm < 2;arm++){
    for(unsigned int layer = 0;layer < 8;layer++){
	...
	double peak_q = hit_q;
	unsigned int peak_index = index;
	TMpcExHit* peak_hit = hit;
	...
	while(!find_peak){
	  ...
	  if(peak_index = index){ 
            ...
            index = max_index;
	    nx = mpcex_hitmap.get_nx(max_hit);
	    ny = mpcex_hitmap.get_ny(max_hit);
	    //search if the hits has alread has a peak belong to 
	    map::<unsigned int,unsigned int> iterator iter = hits_peak_map.find(index); 
	    if(hits_peak_map.find(index) == hits_peak_map.end()){
              hits_in_peak.push_back(index); 
	    }
...
//	  print_surround(hit,mpcex_hitmap);
	  ...
	  set<unsigned int> peak_used_list;
...
//	        if out of range, the index will give 0,if hit0 is null, the good_q wiil give -9999.0
		...
		double hit1_q = get_good_q(hit1);
                
		//will terminate when hit1 is Null or bad q or is bigger than hit0
		if(((hit1_q > 0) && (hit1_q > hit0_q)) || (hit1_q < 0)){ 
                  x_y[j] = true;
		  ...
		  if(hit0_q > 0){
		    if(used_list.find(index0) == used_list.end()){
		      ...
		      peak_used_list.insert(index0);
		      x_y_limit[j] = i;
		    }
		    else{
                      mpcex_peak_hits.status = MpcExPeakHits::Mixed;
		    }
		  }
		  else x_y_limit[j]=i-1;
		}
                
		//not terimate only if hit0_q > hit1_q
		if((hit0_q > 0) && (hit0_q >= hit1_q)){
                  if(used_list.find(index0) == used_list.end()){
		    mpcex_peak_hits.push_back(*hit0);
		    used_list.insert(index0);
		    peak_used_list.insert(index0);
	  ...
	  for(unsigned int ii = 0;ii < 4;ii++){
		...
		if(iy == x_y_pattern[ii][1]+1) index1 = 0;
                TMpcExHit* hit1 = mpcex_hitmap.get_hit(index1);
		
		unsigned int index2 = mpcex_hitmap.get_index(arm,layer,nx+ix*x_y_sign2[ii][0],ny+(iy-1)*x_y_sign2[ii][1]);
                if(ix == x_y_pattern[ii][0]+1) index2 = 0;
		TMpcExHit* hit2 = mpcex_hitmap.get_hit(index2);

                if((iy == x_y_pattern[ii][1]+1) || (ix == x_y_pattern[ii][0]+1)){
                  if(peak_used_list.find(index1) == peak_used_list.end()) hit1 = NULL;
		  if(peak_used_list.find(index2) == peak_used_list.end()) hit2 = NULL;
		}
...
//	    cout <<"status: Good"<<endl;
	  ...
	  if(mpcex_peak_hits.size() > 1) {
	    ...
	    cout <<endl;
	    for(unsigned int ibug2 = 0;ibug2 < mpcex_peak_hits.size();ibug2++){
              TMpcExHit* p = &mpcex_peak_hits[ibug2];
	      cout<<"hit q "<<get_good_q(p)<<endl;
	    }
	    cout<<"-----------------------------------------"<<endl;
	  }
